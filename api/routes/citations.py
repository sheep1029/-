...
{{"title": "论文标题", "authors": ["作者1", "作者2"], "year": 2024, "journal": "期刊名"}}"""
        ...
        if metadata and verified:
            ...
            title_key = "".join([w.lower() for w in title.split()[:3] if w.isalpha()])
            bibtex_key = f"{first_author_last_name}{year}{title_key}"
            ...
            if arxiv_id:
                ieee_citation = f'{author_str}, "{title}," *{journal}*{vol_ieee}{issue_ieee}{pages_ieee}, {year}, arXiv:{arxiv_id}.'
            elif doi:
                ieee_citation = f'{author_str}, "{title}," *{journal}*{vol_ieee}{issue_ieee}{pages_ieee}, {year}, doi: {doi}.'
            else:
                ieee_citation = f'{author_str}, "{title}," *{journal}*{vol_ieee}{issue_ieee}{pages_ieee}, {year}.'
